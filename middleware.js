...
import { allowedAccounts, readSession } from './server/auth/core.js';
...
export default async function middleware(request) {
  ...
  const allowed = allowedAccounts({
    GODSEYE_ALLOWED_EMAILS: process.env.GODSEYE_ALLOWED_EMAILS,
    GODSEYE_ALLOWED_DOMAINS: process.env.GODSEYE_ALLOWED_DOMAINS,
  });
  if (await readSession(request.headers.get('cookie'), process.env.GODSEYE_SESSION_SECRET, allowed)) return next();
