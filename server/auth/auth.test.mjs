...
import { handleAuth, resetDiscoveryCache } from './routes.js';
...
const config = { issuer: ISSUER, clientId: CLIENT_ID, secret: SECRET };
...
async function signIn({ embedNonce } = {}) {
  ...
  const fetchImpl = fakeSysop(async (form) => {
    ...
    return { id_token: await idToken({ nonce: authorize.searchParams.get('nonce') }) };
  });
